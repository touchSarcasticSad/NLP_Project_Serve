async function run(inpt) {
  ...
  document.getElementById("prediction").innerHTML = pred_text  + classes[i] + " with a confidence of " + output[i].toFixed(4);     
}

const pred_text = "The input is : ";
var model = null;

async function loadModel(){
  /* Pretrained in python tensorflow */
  model = await tf.loadLayersModel('/public/static/model.json');
...
$(document).ready(function(){
    loadModel();
    readTextFile("/public/static/word_dict.json", function(text){
